...
intervention_probabilities <- function(x,
  ...
  if (refit || NCOL(x$cumulative_intervention_probs[[protocol_name]]) < length(G_names)){
    ...
    if(intervention_type=="stochastic"){
      ...
      # I need to save the stochastic probability where I intervene only the specific time
      stochastic_ratio.1<-stochastic_probs
      stochastic_ratio.0<-stochastic_probs
    }
        ...
        for (G in current_G_variables){
          ...
          if (refit || length(x$models[[protocol_name]][[G]]$fit) == 0){
            if (G %in% current_constants){
              ...
              # to move this down cause predicted_values was a matrix if for a multinomial, and we only need one
              ...
              if (!(G %in% censoring_variables)){ # then G is a treatment variable
                levels <- x$names$treatment_options[[sub("_[0-9]+$","",G)]]
                if(intervention_type=="stochastic"){
                  ...
                  ## For the targeting step later,
                  ## we also have to create the clever covariate where at this specific time I have to intervene for both,
                  ## and the previous time I just consider the observed ones:
                  # as output we have the P(A=1 | X) for both, the propensity score and the stochastic probs
                  #stochastic_ratio.1[outcome_free_and_uncensored][[G]]<- prob_stochastics/(predicted_values)
                  #stochastic_ratio.0[outcome_free_and_uncensored][[G]]<- 1-prob_stochastics/(1-predicted_values)
                  ...
                  #browser()
                    ...
                    ## the observed value in the current data, we could actually ask to do it also for the binary case:
                    ## FIX ME: this would work only with 0 or 1
                  if(length(ind_trt0<-which(intervened_data[, ..G ]==levels[1]))>0){ # ref level in the specific treatment
                  #print("enter and",ind_trt0[1:5],"")
                  ...
                  if(length(x$names$treatment_options)>2){
                    predicted_values<-predicted_values[,match(intervention_table[time == j,value],x$names$treatment_options)]}
                  else{
                  if(length(ind_trt0<-which(intervened_data[, ..G] %in% levels[1]))>0){predicted_values[ind_trt0] <- 1 - predicted_values[ind_trt0]}
                  }
    ...
    if(intervention_type=="dynamic"){
      ...
      for(j in eval_times){
        ...
        for (v in 1:length(intervention_variables)){ # this for is needed in case of multiple intervention variables
          intervention_match[,j+1] <- previous <- previous*(intervention_values_tab[[intervention_variables[[v]]]] == observed_values[[intervention_variables[[v]]]] )
          intervention_match[,j+1]<-ifelse(is.na(intervention_match[,j+1]),0,intervention_match[,j+1])
          }
