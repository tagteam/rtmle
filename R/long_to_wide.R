...
long_to_wide <- function(x,
    ...
    if ("outcome_data" %in% names(x$long_data)) {
        ...
        stop("long_to_wide: Object does not contain outcome data.")
    }
    competing_data <- NULL
    ...
    if (NROW(outcome_for_widen) == 0L) {
        stop("long_to_wide: Object does not contain outcome dates or all outcome dates are administratively censored.")
    }
