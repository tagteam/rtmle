...
"prepare_data<-" <- function(x,...,value){
    ...
    stopifnot("intervals"%in%names(value))
    ...
    for (protocol_name in names(x$protocols)){
        if (length(x$prepared_data[[protocol_name]]) == 0){
            ...
            if (length(x$data) == 0 || reset){
                ...
                x$data$treatment_data <- map_grid(grid=grid,
                                                  data=x$long_data$treatment_data,
                                                  name=treatment_variables,
                                                  fun.aggregate = funA,
            ...
            x$prepared_data[[protocol_name]]$data <- work_data[]
            x$prepared_data[[protocol_name]]$name_time_covariates <- name_time_covariates
            x$prepared_data[[protocol_name]]$name_baseline_covariates <- name_baseline_covariates
            x$prepared_data[[protocol_name]]$name_constant_variables <- constant_variables
            x$prepared_data[[protocol_name]]$Anodes = intersect(A_nodes, names(work_data))
            x$prepared_data[[protocol_name]]$Cnodes = intersect(C_nodes, names(work_data))
            x$prepared_data[[protocol_name]]$Dnodes = intersect(D_nodes, names(work_data))
            x$prepared_data[[protocol_name]]$Lnodes = intersect(L_nodes, names(work_data)) 
            x$prepared_data[[protocol_name]]$Ynodes = intersect(Y_nodes, names(work_data))
        }
    }
    x
}
