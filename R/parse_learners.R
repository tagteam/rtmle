...
##' parse_learners(c(list("glmnet",
##'                  "glm"=list(learn_variables="A")),list("learn_ranger"),
##'                  list(list(learn_fun="learn_ranger",num.trees=5))))
##' @export 
...
parse_learners <- function(learners){
    ...
    if (length(learner_names) == 0) names(learners) <- learner_names <- rep("noname_learner",length(learners))
