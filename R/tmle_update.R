...
## Version:
## Last-Updated: 21 May 2025 (08:05)
##           By: Alessandra
##     Update #: 47
#----------------------------------------------------------------------
##
### Commentary: Added the definition for the stochastic intervention, where
###             the weights do not need to be inverted and the imatch=1 everywhere by construction
##
### Change Log:
#----------------------------------------------------------------------
##
...
tmle_update <- function(Y,
  ...
  if(intervention_type=="stochastic"){
    # for a stochastic intervention we do not need to invert, cause it is already correctly defined
    # and intervention_match must be 1 everywhere
    weights[subjects_with_weights] <- intervention_probs[subjects_with_weights]
  }
  else{
