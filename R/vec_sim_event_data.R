...
vec_sim_event_data <- function(N,                      # Number of individuals
  ...
  while(!all(Delta %in% term_deltas)){
    # Simulate time
    V <- stats::runif(N)
    W_k1 <- sapply(-log(V), function(u) inverse_sc_haz(u, T_k1, k, L[k + 1]))
    T_k1 <- T_k1 + W_k1

    # Simulate event
    Deltas <- sapply(T_k1, function(t) sample(x, size = 1, prob = probs(t, k, L)) - 1)
