...
## Version:
## Last-Updated: May 21 2025 (16:41)
##           By: Alessandra
##     Update #: 293
#----------------------------------------------------------------------
##
### Commentary: include the Stochastic/dynamic interventions
##              stochastic intervention is onlny for one treatment!
##
### Change Log:
#----------------------------------------------------------------------
##
...
sequential_regression <- function(x,
  ...
  ## this works only for the stochastic intervention, in case of only one treatment.
  t.levels=factor(x$names$treatment_options[[1]], levels=x$names$treatment_options[[1]])
  ...
  for (j in reverse_time_scale){
    ...
    if(intervention_type=="stochastic"){
      # for the stochastic intervention we need to average on the all values of A respect to d(), the intervene function
      # we have to calculate the predicted Y on the observed data:
      intervened_data<- x$prepared_data[,intervenable_history,with = FALSE]

    }
    else{
      # here in the current data, we change all variables as in the intervene_table with their values
      # so for a multiple treatment we will intervene on all the treatment
      # still not sure why we need the time j, since it is not used in the "intervene" function
      # the time will be useful for the dynamic intervention
    ...
    if(intervention_type=="stochastic"){
      ## this would only work for one treatment at the time at the moment
      treatment_time<-intervention_table[time == j-1]$variable
      N.row<-nrow(intervened_data[outcome_free_and_uncensored,])
      ## we need the prediction in the observed data and in a=0,1
      intervened_data.stoch=rbind(intervened_data[outcome_free_and_uncensored,],
                            copy(intervened_data[outcome_free_and_uncensored,])[, (treatment_time):=t.levels[2]],
                            copy(intervened_data[outcome_free_and_uncensored,])[, (treatment_time):=t.levels[1]])

      args <- list(character_formula = interval_outcome_formula,
                   data = current_data[,!(names(current_data) %in% current_constants),with = FALSE],
                   intervened_data = intervened_data.stoch)
    ...
    Q.cont1<-Q.cont0<-g.star<-rep(NA,length(Y))
    if(intervention_type=="stochastic"){
      ## we will have three vectors for the predictions:
      # counterfactuals: c(Q1,Q0)
      Q.cont1[outcome_free_and_uncensored]<-lava::logit(as.numeric(fit_last[(N.row+1):(2*N.row)]))
      Q.cont0[outcome_free_and_uncensored]<-lava::logit(as.numeric(fit_last[(2*N.row+1):(3*N.row)]))
      ## I can save here the intervention at this time,
      ## for a=1 I can just call the function, and for a=0 I calculate 1-gstar(a=1)
      g.star[outcome_free_and_uncensored]<-do.call(x$protocol[[protocol_name]]$intervene_function,
                     list(data = intervened_data[outcome_free_and_uncensored,],
                          name.treat=treatment_time,
                          current.time = j-1))
      ## prediction in the observed A:
      fit_last<-as.numeric(fit_last[1:N.row])
      }
    ...
    if (length(x$targets[[target_name]]$estimator) == 0 || x$targets[[target_name]]$estimator == "tmle"){
      ...
      imatch <- (x$intervention_match[[protocol_name]][,intervention_node_name]%in% 1)## this is a vector of T/F (I need to weight only the ones with 1, cause they respect their treatment strategy)
      if(intervention_type!="stochastic"){
        ## for the stochastic intervention you might not have this problem, apart if a propensity score is = 0
        ## and there is a truncation going on later (FIX THIS, DISCUSS WITH THOMAS)
      ...
      if (inherits(try(
        W <- tmle_update(Y = Y,
                         ...
                         Q.cont1=Q.cont1,Q.cont0=Q.cont0,g.star=g.star,
                         intervention_type=intervention_type) ##for the stochastic intervention we do not define the inverse, because it is based already on the cumulative product with prop score at the den.
      ),"try-error"))
      ...
      W <- fit_last
      ## if no updating step, then in case of stochastic intervention
      ## we have to do the marginalization over g*
      if(intervention_type=="stochastic"){W<- Q.cont1*g.star + Q.cont0*(1-g.star)}
    }
    ...
    if(intervention_type=="stochastic"){
      h.g.ratio<- x$cumulative_intervention_probs[[protocol_name]][,ipos] }
    else{
      h.g.ratio <- 1/x$cumulative_intervention_probs[[protocol_name]][,ipos]
    }

    if (any(h.g.ratio>10)) h.g.ratio <- pmin(h.g.ratio,10)
    if (length(x$names$censoring)>0){
