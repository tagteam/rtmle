...
#' x <- discretize_data(x,L=list(method="locf"),A=list(method="exposure_percent"),
#'                 baseline_lookback=180)
#' x$data$timevar_data$L
#' x$data$timevar_data$V
#' x <- discretize_data(x,L=list(method="measurement",fun_aggregate="median"))
#' x$data
...
discretize_data <- function(x,
    ...
    if (length(x$data$baseline_data) == 0){
        stop("To use discretize_data on the long-format data we need the baseline data stored as 'x$data$baseline_data' with the subject id variable.\nUse the function 'add_baseline_data' to add this.")
    }
    ...
    if (length(Vnames) > 0){
        ...
        if ((hit <- match("breaks", names(dots), nomatch = 0)) > 0){
            warning("rtmle::discretize_data: Argument 'breaks' is obsolete. Break points are stored in the object as x$time_grid and x$time_grid_scale.")
            dots <- dots[-hit]
        }
        if (length(dots) == 0) {
            dots <- NULL
    ...
    if ("outcome_data" %in% names(x$long_data)) {
        ...
        stop("discretize_data: Object does not contain outcome data.")
    }
    competing_data <- NULL
    ...
    if (NROW(outcome_for_widen) == 0L) {
        stop("discretize_data: Object does not contain outcome dates or all outcome dates are administratively censored.")
    }
