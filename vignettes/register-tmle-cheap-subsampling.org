...
estimators for the analysis of longitudinal register data.  In this
vignette, we detail how to use cheap subsampling with the =rtmle=
package to obtain robust inference. In particular, one may want to
supplement the analysis with cheap bootstrap confidence intervals if
the asymptotic confidence intervals are suspiciously narrow or
suspiciously wide. Cheap subsampling has been demonstrated to have
good coverage readily with a few bootstrap data sets, see
https://arxiv.org/abs/2501.10289. Furthermore, we highlight two
important features of using cheap subsampling with =rtmle=:
...
#+BEGIN_SRC R  :results output verbatim  :exports both  :session *R* :cache yes
  ...
  x <- discretize_data(x)
  x <- regime(x,name = "Always_A",intervention = data.frame("A" = factor("1",levels = c("0","1"))),verbose = FALSE)
  x <- regime(x,name = "Never_A",intervention = data.frame("A" = factor("0",levels = c("0","1"))),verbose = FALSE)
...
#+begin_example
          Target Regime Time_horizon Bootstrap_standard_error Bootstrap_lower Bootstrap_upper Target_parameter Estimator  Estimate
...
#+begin_example
          Target Regime Target_parameter Time_horizon Estimator   Estimate   P_value Standard_error      Lower     Upper
...
library(ggplot2)
ggplot2::ggplot(
    data = x$estimate$Cheap_bootstrap[Target_parameter=="Risk" & Regime=="Always_A"],
