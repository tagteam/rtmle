...
The function *regime* is needed as first step to define the type of intervention of interest.
Here we specify:

 - *name* : name of the regime 
 - *treatment_variables*:  name(s) of the variable(s) that the regimes intervenes upon
 ...
 In case of a dynamic intervention, a function needs to be specified through
 *intervene_function*. For a history-dependent treatment rule, declare
 *adherence_model_strata* to request separate adherence models in pre-decision
 subgroups; omit it for one pooled dynamic adherence model. The treatment
 factorization is selected separately with *multiple_treatment_factorization*.
 

### Examples: 

#### Static Intervention: One single binary variable exposure (Always treated regime)

```{r}
x <- regime(x,name = "always A",treatment_variables = "A",
intervention = 1)
```
...
```{r}
x <- regime(x,name = "always A",treatment_variables = c("A_1","A_2","A_3"),
intervention = 1)
```
...
```{r}
x <- regime(x,name = "always combined",treatment_variables = list("A"=c("A_1","A_2","A_3"),"B"=c("B_1","B_2","B_3")),
...
```{r}
x <- regime(x,name = "always combined",treatment_variables = matrix(c("A_1","A_2","A_3", "B_1","B_2","B_3"),3),
...
```{r}
x <- regime(x,name = "always combined", treatment_variables = c("A","B"),
intervention = c(1,1))
```
