...
test_that("run rtmle on simulated data",{
    ...
    x <- protocol(x,name = "Sometimes_A",treatment_variables = "A",intervention = data.table(time = c(0,1,2,3), A = factor(c(1,1,NA,NA), levels = c(0,1))))
    x <- target(x,name = "Outcome_risk",estimator = "tmle",protocols = "Sometimes_A")
    x <- model_formula(x)
