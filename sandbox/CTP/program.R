...
prepare_data(x) <- list()
protocol(x) <- list(name = "hurz",treatment_variables = "treatment",intervention = 1)
protocol(x) <- list(name = "blanko",treatment_variables = "treatment",intervention = 0)
target(x) <- list(name = "Mine",strategy = "additive",estimator = "tmle",protocols = c("hurz","blanko"))
x <- run_rtmle(x)
x
