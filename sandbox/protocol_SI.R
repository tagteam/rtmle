...
#----------------------------------------------------------------------
## ### Code:
##' Define a named regime for a hypothetical/emulated trial
##'
##' This function adds a regime to an existing object.
##' A regime defines the values of the treatment variable(s)
...
##' \itemize{
##' \item \code{name}: the name of the regime
##' \item \code{variable}: the name(s) of the variable(s) that the regimes intervenes upon
##' \item \code{intervention}: function which determines the value(s) of the variable(s) under the intervention
##' }
##' @export
"regime<-" <- function(x,...,value) {
  ...
  x$regimes[[value$name]]$intervention_table <- data.table(time = intervention_times,
                                                             variable = paste0(value$treatment_variables,"_",intervention_times),
                                                             value = value$intervention)
  x$regimes[[value$name]]$treatment_variables <- value$treatment_variables
  if (length(value$intervene_function)>0){
    x$regimes[[value$name]]$intervene_function <- value$intervene_function # here we define the function for the stochastic
  }else{
    x$regimes[[value$name]]$intervene_function <- "intervene"
  }
  # we have to save somewhere which kind of intervention we are considering
  x$regimes[[value$name]]$intervention_type<-value$intervention_type
  x
}
######################################################################
### regime.R ends here
