...
x <- model_formula(x)
system.time(x <- run_rtmle(x,refit = TRUE,time_horizon = 2,learner = c("learn_glm","learn_glm")))
...
summary(x)
system.time(x <- run_rtmle(x,refit = TRUE,time_horizon = 2,learner = c("learn_glm","learn_glmnet","learn_ranger")))
