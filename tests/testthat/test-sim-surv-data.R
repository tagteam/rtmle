...
test_that("sim_surv_data simulates data in the right way",{
  ...
  expect_true(confint(survfit_death)[2,1] <= beta[2,1] & beta[2,1] <= confint(survfit_death)[2,2])

  survfit_cens <- coxph(Surv(Time, Delta == 2) ~ L0 + A, data = data)
  expect_true(confint(survfit_cens)[1,1] <= beta[1,2] & beta[1,2] <= confint(survfit_cens)[1,2])
  expect_true(confint(survfit_cens)[2,1] <= beta[2,2] & beta[2,2] <= confint(survfit_cens)[2,2])
})
