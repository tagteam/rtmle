...
test_that("sim_op_data simulates data in the right way",{
  ...
  data_test <- sim_op_data(10^3, beta = beta)
  ...
  # Fit models
  survfit_cens <- coxph(Surv(tstart, tstop, Delta == 0) ~ L0 + A0 + L + A, data = data_int, cluster = ID)
  survfit_death <- coxph(Surv(tstart, tstop, Delta == 1) ~ L0 + A0 + L + A, data = data_int, cluster = ID)
  survfit_oper <- coxph(Surv(tstart, tstop, Delta == 2) ~ L0 + A0 + L, data = data_int[at_risk_oper == 1], cluster = ID)
  survfit_cov <- coxph(Surv(tstart, tstop, Delta == 3) ~ L0 + A0 + A, data = data_int[at_risk_cov == 1], cluster = ID)

  # Compare confidence intervals and true values
  expect_true(confint(survfit_oper)[1,1] <= beta[1,3] & beta[1,3] <= confint(survfit_oper)[1,2])
  expect_true(confint(survfit_oper)[2,1] <= beta[2,3] & beta[2,3] <= confint(survfit_oper)[2,2])
  expect_true(confint(survfit_oper)[3,1] <= beta[3,3] & beta[3,3] <= confint(survfit_oper)[3,2])
  ...
  expect_true(confint(survfit_cens)[3,1] <= beta[3,1] & beta[3,1] <= confint(survfit_cens)[3,2])
  expect_true(confint(survfit_cens)[4,1] <= beta[4,1] & beta[4,1] <= confint(survfit_cens)[4,2])
  ...
  expect_true(confint(survfit_cov)[3,1] <= beta[4,4] & beta[4,4] <= confint(survfit_cov)[3,2])
})
