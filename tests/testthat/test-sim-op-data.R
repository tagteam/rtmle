...
test_that("sim_op_data simulates data in the right way",{
  ...
  expect_true(confint(survfit_oper)[2,1] <= beta[3,1] & beta[3,1] <= confint(survfit_oper)[2,2])
  expect_true(confint(survfit_oper)[3,1] <= beta[4,1] & beta[4,1] <= confint(survfit_oper)[3,2])
