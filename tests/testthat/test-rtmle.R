...
test_that("run rtmle on simulated data",{
    ...
    x <- long_to_wide(x,intervals = seq(0,2000,30.45*6),verbose = 0L)
...
test_that("obsolete way of run rtmle on simulated data",{
    set.seed(112)
    ld <- simulate_long_data(n = 1000,number_visits = 20,beta = list(A_on_Y = -.2,A0_on_Y = -0.3,A0_on_A = 6),register_format = TRUE)
    old <- rtmle_init(intervals = 2,name_id = "id",name_outcome = "Y",name_competing = "Dead",name_censoring = "Censored",censored_label = "censored")
    old <- add_long_data(old,outcome_data = ld[["outcome_data"]],censored_data = ld[["censored_data"]],competing_data = ld[["competing_data"]],timevar_data = ld[["timevar_data"]])
    old <- add_baseline_data(old,data = ld$baseline_data[,start_followup_date := 0])
    old <- long_to_wide(old,intervals = seq(0,2000,30.45*6),verbose = FALSE)
    old <- protocol(old,name = "Always_A",treatment_variables = "A",intervention = 1)
    old <- prepare_data(old) 
    old <- target(old,name = "Outcome_risk",estimator = "tmle",protocols = "Always_A")
    old <- model_formula(old)
    suppressWarnings(old <- run_rtmle(old,verbose = FALSE))
    expect_equal(old$estimate$Main_analysis$Estimate,0.1893173,tolerance = 0.001)
    expect_equal(old$estimate$Main_analysis$Standard_error,0.01976462,tolerance = 0.001)
})
