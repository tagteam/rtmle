...
## Version: 
## Last-Updated: Jul 30 2024 (10:59) 
...
# no effect of A
set.seed(18124)
ld0 <- simulate_long_data(n = 100000,number_epochs = 20,beta = list(A0_on_A = 6),register_format = TRUE)
...
# effect on Y
expect_equal(diff(as.numeric(unlist(predict(fit0,times = 800,cause = "Y",newdata = data.frame(A_0 = c(0,1)))))),0,tolerance = 0.001)
expect_equal(diff(as.numeric(unlist(predict(fitA0,times = 800,cause = "Y",newdata = data.frame(A_0 = c(0,1)))))),-0.09647,tolerance = 0.001)
expect_equal(diff(as.numeric(unlist(predict(fitA,times = 800,cause = "Y",newdata = data.frame(A_0 = c(0,1)))))),-0.04572,tolerance = 0.001)
expect_equal(diff(as.numeric(unlist(predict(fitA0A,times = 800,cause = "Y",newdata = data.frame(A_0 = c(0,1)))))),-0.13316,tolerance = 0.001)
