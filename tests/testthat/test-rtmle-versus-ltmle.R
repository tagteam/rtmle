...
if (requireNamespace("ltmle")){
    ...
    test_that("single time point compare rtmle with ltmle",{
        ...
        suppressMessages(x <- regime(x,name = "A",treatment_variables = "A",intervention = 1))
        x <- target(x,name = "Outcome_risk",estimator = "tmle",regimes = "A")
        x <- model_formula(x)
    ...
    test_that("longitudinal data compare rtmle with ltmle",{
        ...
        x <- regime(x,name = "Never_A",treatment_variables = "A",intervention = 0)
    ...
    if (FALSE){
        test_that("longitudinal data compare rtmle with ltmle two-dimensional treatment",{
            ...
            x <- prepare_rtmle_data(x)
            x <- regime(x,name = "AnotB",treatment_variables = c("A","B"),intervention = c(1,0))
            x <- regime(x,name = "BnotA",treatment_variables = c("A","B"),intervention = c(0,1))
            x <- target(x,name = "Outcome_risk",estimator = "tmle",regimes = c("AnotB","BnotA"))
